...
function easyGame() {
  ...
  tmp_content = [
    'Es war ein schöner Montagmorgen in Berlin.',
...
function finishGame() {
  if (content_counter == tmp_content.length) {
    ...
    if (delta < current_best_time) {
      ...
      finished_section.innerText =
        'Congrats. Your new best time is ' +
        String(minutes) +
        ' minutes and ' +
        String(seconds) +
        ' seconds. Can you beat this?';
    } else {
      finished_section.innerText =
        "You didn't beat your highscore. Your time is " +
        String(minutes) +
        ' minutes and ' +
        String(seconds) +
        ' seconds. Can you beat this?';
...
function startGame() {
  ...
  // }
  start_btn.innerHTML =
    '<img src="https://www.filepicker.io/api/file/9KBTW5kDRluhcmsNlIOE" width="50" height="50">';
  // resetGame();
  ChangeContent();
  start_counter++;
